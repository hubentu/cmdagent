...
To create an API, import the `tool_api` function from `cwlagent.remote_api` and pass in the path to a CWL file and the name of the tool:
```python
...
To create a tool agent, import the `cwlagent` function from `cwlagent.agent` and pass in the API instance:
```python
